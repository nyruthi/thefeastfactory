...
async function refreshCustomerSession(refreshToken: string): Promise<CustomerSession> {
  if (!refreshPromise) {
...
export async function apiRequest<T>(path: string, init: RequestInit = {}, token?: string): Promise<T> {
  ...
  const currentToken = token && storedSession ? storedSession.accessToken : token;
  let response = await request(path, init, currentToken);

  if (response.status === 401 && token && storedSession?.refreshToken) {
    try {
      const refreshedSession = await refreshCustomerSession(storedSession.refreshToken);
