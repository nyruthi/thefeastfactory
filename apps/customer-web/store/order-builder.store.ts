...
export const useOrderBuilderStore = create<OrderBuilderState>()(
  persist(
    (set, get) => ({
      ...
      setPackage: (pkg) =>
        set((state) => {
          if (state.package?.packageVersionId === pkg.packageVersionId) return { package: pkg };
          return {
            package: pkg,
            event: undefined,
      ...
      toggleItem: (item, maxSelections) => {
        const state = get();
        const exists = state.selectedItems.some((selected) => selected.menuItemId === item.menuItemId);
        if (exists) {
          set({ selectedItems: state.selectedItems.filter((selected) => selected.menuItemId !== item.menuItemId) });
          return true;
        }
      ...
      removeItem: (menuItemId) =>
        set((state) => ({
          selectedItems: state.selectedItems.filter((selected) => selected.menuItemId !== menuItemId),
        })),
      clearSelections: () => set({ selectedItems: [] }),
      reset: () => set({ package: undefined, event: undefined, guestCount: 10, selectedItems: [] }),
    }),
    {
