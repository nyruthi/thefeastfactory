import { ArrowRight, CalendarDays, CheckCircle2, IndianRupee, Sparkles, Utensils } from 'lucide-react';
import Link from 'next/link';
import { Button } from '../components/ui/button';

const steps = [
  { label: 'Tell us the occasion', copy: 'Choose your date, venue, and guest count.', icon: CalendarDays },
  { label: 'Curate the menu', copy: 'Build within a package, then add premium favourites.', icon: Utensils },
  { label: 'Confirm with clarity', copy: 'See the exact per-plate and event total before payment.', icon: IndianRupee },
];
    ...
    <main>
          ...
          <div className="relative grid items-center gap-12 lg:grid-cols-[1.25fr_0.75fr]">
            <div>
              <p className="flex items-center gap-2 text-xs font-bold uppercase tracking-[0.22em] text-accent">
                <Sparkles className="h-4 w-4" /> Thoughtful catering for every gathering
              </p>
              ...
              <p className="mt-6 max-w-2xl text-base leading-7 text-white/70 sm:text-lg">
                Choose a crafted package, personalize every course, and get transparent pricing for your
                celebration in minutes.
              </p>
              <div className="mt-9 flex flex-wrap gap-3">
                <Button asChild variant="secondary">
                  <Link href="/packages">Explore packages <ArrowRight className="ml-2 h-4 w-4" /></Link>
                </Button>
            ...
            <div className="surface-card relative overflow-hidden border-white/10 bg-white/10 p-6 text-white backdrop-blur-xl">
              ...
              <div className="mt-8 rounded-xl bg-white/10 p-5">
                ...
                <p className="mt-1 text-sm text-white/60">Guests, with packages that scale with you</p>
              </div>
            </div>
          </div>
        </div>
      ...
      <section className="page-shell pt-4">
        <div className="max-w-2xl">
          ...
          <h2 className="mt-3 font-serif text-4xl font-semibold">From first idea to final plate.</h2>
        </div>
        <div className="mt-8 grid gap-4 md:grid-cols-3">
          {steps.map((step, index) => (
            <article key={step.label} className="surface-card group p-6 transition hover:-translate-y-1">
              <div className="flex items-center justify-between">
                <span className="grid h-12 w-12 place-items-center rounded-full bg-primary/8 text-primary">
                  <step.icon className="h-5 w-5" />
                </span>
                <span className="font-serif text-3xl text-border">0{index + 1}</span>
              </div>
              <h3 className="mt-8 font-serif text-2xl font-semibold">{step.label}</h3>
