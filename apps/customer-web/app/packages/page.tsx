    ...
    <main className="page-shell pb-28">
      <div className="grid gap-6 lg:grid-cols-[1fr_auto] lg:items-end">
        <div className="max-w-3xl">
          ...
          <h1 className="mt-3 font-serif text-5xl font-semibold">
            A generous starting point for every occasion.
          </h1>
          <p className="mt-4 max-w-2xl leading-7 text-muted-foreground">
            Each package includes a balanced menu structure. Pick your dishes,
            see any premium additions, and stay in control of the final price.
          </p>
        </div>
        <div className="rounded-full border bg-white/70 px-5 py-3 text-sm text-muted-foreground">
          <Users className="mr-2 inline h-4 w-4 text-primary" /> Built for
          gatherings from 10 guests
        </div>
      </div>
      ...
      {!error && !packages.length && (
        <div className="mt-10 grid gap-4 md:grid-cols-3">
          {[1, 2, 3].map((item) => (
            <div
              key={item}
              className="h-80 animate-pulse rounded-xl bg-white/60"
            />
          ))}
        </div>
      )}
      ...
      <div className="mt-10 grid gap-5 md:grid-cols-2 xl:grid-cols-3">
        {packages.map((pkg, index) => (
          ...
          >
            <div
              className={`h-2 ${index % 3 === 1 ? 'bg-accent' : index % 3 === 2 ? 'bg-[#9c5940]' : 'bg-primary'}`}
            />
            <div className="flex flex-1 flex-col p-7">
              ...
              <p className="mt-4 line-clamp-3 text-sm leading-6 text-muted-foreground">
                {pkg.description ||
                  'A thoughtfully balanced catering menu ready to personalize for your event.'}
              </p>
              <div className="mt-6 space-y-2 text-sm">
                {(pkg.isCustom
                  ? ['Any active menu item', 'Actual item pricing']
                  : ['Flexible course selection', 'Live customization pricing']
                ).map((feature) => (
