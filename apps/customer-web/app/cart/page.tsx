  ...
  const selectionStatus = useMemo(() => {
    ...
    if (config.isCustom)
      return {
        ...
        missing: selectedItems.length ? [] : ['at least one dish'],
      };
      ...
      <main className="page-shell">
        <div className="surface-card mx-auto max-w-xl p-8 text-center sm:p-12">
          ...
          <h1 className="mt-6 font-serif text-4xl font-semibold">
            Your cart is ready for an idea.
          </h1>
          <p className="mt-3 leading-7 text-muted-foreground">
            Start with a catering package, then add your event and curate the
            menu.
          </p>
      ...
      <div className="mt-8 flex flex-wrap items-end justify-between gap-4">
        <div>
          ...
          <h1 className="mt-3 font-serif text-5xl font-semibold">
            Review the celebration plan.
          </h1>
        </div>
        <button
          onClick={reset}
          className="flex items-center gap-2 text-sm font-semibold text-muted-foreground hover:text-red-700"
        >
          ...
          <section className="surface-card p-6">
            <div className="flex flex-wrap items-start justify-between gap-4">
              ...
              <Button asChild variant="outline">
                <Link href="/packages">Change package</Link>
              </Button>
            </div>
          </section>
            ...
            <div className="flex items-center justify-between gap-4">
              <div>
                ...
                <h2 className="mt-2 font-serif text-2xl font-semibold">
                  {event?.eventName ||
                    (event ? 'Your catering event' : 'Not added yet')}
                </h2>
              </div>
              <Button asChild variant="outline">
                <Link
                  href={`/events/new?packageVersionId=${cartPackage.packageVersionId}`}
                >
                  {event ? 'Edit' : 'Add details'}
                </Link>
              </Button>
            </div>
            {event ? (
              <div className="mt-5 grid gap-3 text-sm text-muted-foreground sm:grid-cols-3">
                ...
                <p className="flex gap-2">
                  <Users className="h-4 w-4 text-primary" /> {guestCount} guests
                </p>
                <p className="flex gap-2">
            ...
            <div className="flex items-center justify-between gap-4 border-b bg-white/60 px-6 py-5">
              <div>
                ...
                <h2 className="mt-1 font-serif text-2xl font-semibold">
                  {selectedItems.length} dishes selected
                </h2>
              </div>
              <Button asChild variant="outline">
                <Link href="/menu/select">
                  {selectedItems.length ? 'Edit menu' : 'Choose dishes'}
                </Link>
              </Button>
            </div>
                      ...
                      {items.map((item) => (
                        ...
                        >
                          ...
                          <button
                            ...
                            className="grid h-9 w-9 place-items-center rounded-full text-muted-foreground hover:bg-red-50 hover:text-red-700"
        ...
        <aside className="surface-card h-fit p-7 lg:sticky lg:top-28">
          ...
          <div className="mt-6 space-y-3 text-sm">
            {!cartPackage.isCustom && (
              <div className="flex justify-between">
                ...
                <span>₹{Number(cartPackage.basePricePerPlate).toFixed(2)}</span>
              </div>
            )}
            <div className="flex justify-between">
          ...
          {!selectionStatus.valid && event && (
            <p className="mt-4 rounded-lg bg-amber-50 p-3 text-xs leading-5 text-amber-800">
              Complete:{' '}
              {selectionStatus.missing.join(', ') || 'menu selections'}.
            </p>
          )}
          ...
          <p className="mt-3 text-center text-xs text-muted-foreground">
            Final pricing is confirmed securely at checkout.
          </p>
        </aside>
      </div>
    </main>
