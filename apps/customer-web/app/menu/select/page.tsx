...
import { Check, ChevronRight, ImagePlus, Leaf, RotateCcw, ShoppingBag, Utensils, Users, X } from 'lucide-react';
...
type MenuSelectionItem = PackageConfiguration['categoryRules'][number]['items'][number] & {
...
export default function MenuSelectPage() {
  ...
  function selectItem(item: MenuSelectionItem, categoryId: string, categoryName: string, maxSelections: number) {
  ...
  function detailText(item: MenuSelectionItem) {
    return {
      description: item.description || 'A Feast Factory catering favourite prepared fresh for your event menu.',
      ingredients:
        item.ingredients ||
        item.description ||
  ...
  if (error) return <main className="page-shell"><StatePanel tone="danger" title="Menu could not load" description={error} actionHref="/packages" actionLabel="Choose another package" /></main>;
                    ...
                    >
                      ...
                      >
                        <div className="relative aspect-[4/3] bg-muted">
                          {item.imageUrl ? (
                            <img src={item.imageUrl} alt={item.name} className="h-full w-full object-cover transition duration-300 group-hover:scale-[1.03]" />
                          ) : (
                            <div className="grid h-full place-items-center bg-gradient-to-br from-primary/10 via-accent/10 to-white text-primary">
                              <ImagePlus className="h-9 w-9" />
                            </div>
                          )}
                          <span className={`absolute left-3 top-3 inline-flex items-center gap-1 rounded-full px-2 py-1 text-[10px] font-bold uppercase tracking-wide shadow-sm ${item.isVeg ? 'bg-green-50 text-green-700' : 'bg-orange-50 text-orange-700'}`}>
                            <Leaf className="h-3 w-3" /> {item.isVeg ? 'Veg' : 'Non-veg'}
                          </span>
                        </div>
                        <div className="p-4">
                          <strong className="block font-serif text-xl">{item.name}</strong>
                          <p className="mt-2 line-clamp-2 min-h-10 text-sm leading-5 text-muted-foreground">{details.description}</p>
            ...
            <div className="grid md:grid-cols-[0.95fr_1.05fr]">
              <div className="relative min-h-72 bg-muted">
                {detailItem.item.imageUrl ? (
                  <img src={detailItem.item.imageUrl} alt={detailItem.item.name} className="h-full max-h-[92vh] min-h-72 w-full object-cover" />
                ) : (
              ...
              <div className="overflow-y-auto p-6 sm:p-7">
                <div className="flex items-start justify-between gap-4">
                  <div>
                    <span className={`inline-flex items-center gap-1 rounded-full px-2 py-1 text-[10px] font-bold uppercase tracking-wide ${detailItem.item.isVeg ? 'bg-green-50 text-green-700' : 'bg-orange-50 text-orange-700'}`}>
                      <Leaf className="h-3 w-3" /> {detailItem.item.isVeg ? 'Vegetarian' : 'Non-vegetarian'}
                    </span>
                    <h2 className="mt-4 font-serif text-3xl font-semibold">{detailItem.item.name}</h2>
                    <p className="mt-2 text-sm font-semibold text-primary">{detailItem.categoryName}</p>
                  </div>
                  <button
                    type="button"
                ...
                <div className="mt-6 space-y-5">
                  <section>
                    <h3 className="text-sm font-bold uppercase tracking-[0.16em] text-muted-foreground">Description</h3>
                    <p className="mt-2 leading-7 text-muted-foreground">{detailText(detailItem.item).description}</p>
                  </section>
                  <section>
                    <h3 className="text-sm font-bold uppercase tracking-[0.16em] text-muted-foreground">Ingredients</h3>
                    <p className="mt-2 leading-7 text-muted-foreground">{detailText(detailItem.item).ingredients}</p>
                  </section>
                    ...
                    <div className="flex justify-between gap-4">
                      ...
                      <strong>
                        {isCustom
                          ...
                          : Number(detailItem.item.adjustmentAmount) > 0 ? `+₹${detailItem.item.adjustmentAmount} per plate` : 'Included'}
                      </strong>
                    </div>
                  </div>
                  <Button
                  ...
                  >
                    {selectedItems.some((selectedItem) => selectedItem.menuItemId === detailItem.item.id) ? 'Remove from menu' : 'Add to menu'}
                  </Button>
                </div>
              </div>
            </div>
