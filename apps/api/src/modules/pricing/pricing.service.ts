...
export class PricingService {
  ...
  async quote(packageVersionId: string, guestCount: number, selectedItems: SelectedItemInput[]) {
    ...
    for (const rule of version.categoryRules) {
      const count = selectedItems.filter((item) => item.categoryId === rule.categoryId).length;
    ...
    const items = selectedItems.flatMap((selection) => {
      ...
      if (!row || row.categoryId !== selection.categoryId || !row.menuItem.isActive || row.menuItem.deletedAt) {
      ...
      return [{
        ...
        categoryName: version.categoryRules.find((rule) => rule.categoryId === selection.categoryId)?.category.name ?? '',
    ...
    if (errors.length) throw new BadRequestException({ message: 'Invalid package selection', errors });
    const customization = items.reduce((sum, item) => sum.plus(item.adjustmentAmount), new Prisma.Decimal(0));
