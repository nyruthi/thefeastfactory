...
export default function AdminLogin() {
  ...
  return (
    <main className="mx-auto grid min-h-[calc(100vh-4rem)] max-w-5xl gap-8 px-5 py-12 lg:grid-cols-[1fr_420px] lg:items-center">
      <section>
        <p className="text-sm font-semibold uppercase tracking-[0.18em] text-primary">Operations access</p>
        <h1 className="mt-3 font-serif text-5xl font-semibold">Run catering fulfilment from one dashboard.</h1>
        <p className="mt-4 max-w-xl leading-7 text-muted-foreground">
          Manage orders, payments, package rules, menu availability, and event queues with protected staff access.
        </p>
        <div className="mt-8 grid gap-3 sm:grid-cols-3">
          {['Role-based access', 'Payment ledger', 'Fulfilment queue'].map((item) => (
      ...
      <form onSubmit={submit} className="admin-card p-6 sm:p-8">
        ...
        <p className="mt-2 text-sm leading-6 text-muted-foreground">Use your staff credentials to continue.</p>
        ...
        <div className="mt-7 space-y-4">
          ...
          {error && (
            <div role="alert" className="flex gap-3 rounded-xl border border-red-100 bg-red-50 p-3 text-sm text-red-800">
