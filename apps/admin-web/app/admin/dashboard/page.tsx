...
const rangeFormatter = new Intl.DateTimeFormat('en-IN', { day: 'numeric', month: 'short', year: 'numeric' });
...
export default function Dashboard() {
  ...
  const eventsByDay = useMemo(() => {
    ...
    for (const events of grouped.values()) {
      events.sort((a, b) => timeLabel(a.eventTimeStart).localeCompare(timeLabel(b.eventTimeStart)));
    }
    return grouped;
      ...
      <section className="admin-card mt-6">
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div>
            ...
            <p className="mt-1 text-sm text-muted-foreground">{rangeFormatter.format(weekStart)} - {rangeFormatter.format(weekEnd)}</p>
          </div>
          <div className="flex gap-2">
            <Button type="button" variant="outline" onClick={() => setWeekStart(addDays(weekStart, -7))} aria-label="Previous week">
              <ChevronLeft className="h-4 w-4" />
            </Button>
            <Button type="button" variant="secondary" onClick={() => setWeekStart(startOfWeek(new Date()))}>This week</Button>
            <Button type="button" variant="outline" onClick={() => setWeekStart(addDays(weekStart, 7))} aria-label="Next week">
        ...
        <div className="mt-5 grid gap-3 xl:grid-cols-7">
          {weekDays.map((day) => {
            ...
            return (
              <div key={key} className="min-h-[260px] rounded-xl border bg-white p-3">
                <div className="flex items-center justify-between border-b pb-2">
                  <div>
                    <p className="text-sm font-semibold">{dayFormatter.format(day)}</p>
                    <p className="text-xs text-muted-foreground">{dateFormatter.format(day)}</p>
                  </div>
                  <span className="rounded-full bg-muted px-2 py-1 text-xs font-semibold">{dayEvents.length}</span>
                </div>
                <div className="mt-3 space-y-3">
                  {dayEvents.map((event) => {
                    const order = event.orders[0];
                    return (
                      <article key={event.id} className="rounded-xl border-l-4 border-primary bg-muted/45 p-3">
                        <div className="flex items-start justify-between gap-2">
                          <div>
                            <p className="text-xs font-semibold text-primary">{timeLabel(event.eventTimeStart)}</p>
                            <h3 className="mt-1 text-sm font-semibold">{event.eventName || 'Catering event'}</h3>
                          </div>
                        ...
                        {order ? (
                          ...
                          <p className="mt-3 text-xs font-semibold text-muted-foreground">No order placed</p>
                        )}
                      </article>
                    );
                  })}
                  {!dayEvents.length && <p className="pt-8 text-center text-sm text-muted-foreground">No orders</p>}
                </div>
              </div>
            );
          })}
