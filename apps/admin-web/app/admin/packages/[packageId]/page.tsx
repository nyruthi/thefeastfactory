...
export default function AdminPackage() {
  ...
  useEffect(() => {
    if (session)
      apiRequest<any[]>('/admin/packages', {}, session.accessToken).then(
        (rows) => setPkg(rows.find((r) => r.id === packageId)),
      );
  }, [session, packageId]);
